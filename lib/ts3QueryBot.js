    ...
    async start() {    
        try {
            ...
            this.query.on('cliententerview', (data) => {
                if(clientEnterViewBlocked[data.client_unique_identifier])
                    return;

                clientEnterViewBlocked[data.client_unique_identifier] = true;
                
                this.clientJoined(data);
                setTimeout(() => {
                    delete clientEnterViewBlocked[data.client_unique_identifier];
                }, 1000);
            });
